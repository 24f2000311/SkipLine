        ...
        <div>
          ...
          <div className="flex items-center gap-3 text-[13px] font-medium text-slate-500">
            ...
            {isEventEnded && !isEventCancelled && (
              <span className="px-2.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-widest bg-slate-200 text-slate-700 dark:bg-slate-800 dark:text-slate-300">
                Event Ended
              </span>
            )}
            {isEventCancelled && (
              <span className="px-2.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-widest bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-400">
                Event Cancelled
              </span>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 space-y-6 lg:sticky lg:top-24">
          ...
          <div className={`border rounded-2xl p-6 sm:p-8 flex flex-col items-center justify-center text-center gap-6 transition-all ${
            ...
            <div>
              <h2 className="text-2xl font-extrabold text-foreground tracking-tight mb-2">Operations</h2>
            ...
            <Button 
              ...
              className={`w-full h-20 text-xl font-black rounded-xl transition-all shadow-sm ${
          ...
          <div className="grid grid-cols-3 divide-x divide-slate-100 dark:divide-slate-800 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden text-center">
            <div className="p-4 sm:p-5">
              <div className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">{isLoadingEntries ? '-' : waitingCount}</div>
              <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">Waiting</div>
            </div>
            <div className="p-4 sm:p-5 bg-emerald-50/30 dark:bg-emerald-900/5">
              <div className="text-3xl font-black text-emerald-600 dark:text-emerald-400 tracking-tight">{isLoadingEntries ? '-' : calledCount}</div>
              <div className="text-[10px] font-bold text-emerald-600/70 dark:text-emerald-400/70 uppercase tracking-widest mt-1">Called</div>
            </div>
            <div className="p-4 sm:p-5 bg-blue-50/30 dark:bg-blue-900/5">
              <div className="text-3xl font-black text-sl-blue dark:text-blue-400 tracking-tight">{isLoadingEntries ? '-' : servingCount}</div>
              <div className="text-[10px] font-bold text-sl-blue/70 dark:text-blue-400/70 uppercase tracking-widest mt-1">Serving</div>
            </div>
          </div>
          ...
          <div className="bg-slate-50/50 dark:bg-slate-900/20 border border-slate-200 dark:border-slate-800 rounded-2xl p-6">
            ...
            <dl className="grid grid-cols-2 gap-4 text-sm">
              <div className="p-3 bg-white dark:bg-slate-950 rounded-lg border border-slate-100 dark:border-slate-800/60 shadow-sm">
                ...
                <dd className="font-bold text-base text-slate-900 dark:text-white">{queue.estimatedServiceTime} mins</dd>
              </div>
              
              <div className="p-3 bg-white dark:bg-slate-950 rounded-lg border border-slate-100 dark:border-slate-800/60 shadow-sm">
                <dt className="text-xs font-semibold text-slate-500 uppercase tracking-widest mb-1">Policy</dt>
                <dd className="font-bold text-base text-slate-900 dark:text-white line-clamp-1">
